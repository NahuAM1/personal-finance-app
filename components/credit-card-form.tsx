...
export function CreditCardForm({ onSubmit }: CreditCardFormProps) {
  ...
  const monthlyAmount =
    totalAmount && installments ? (Number.parseFloat(totalAmount) / Number.parseInt(installments)).toFixed(2) : "0.00"
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
        <CardHeader>
          <CardTitle className="text-blue-800 dark:text-blue-200">¿Cómo funciona?</CardTitle>
          <CardDescription>
            Los gastos con tarjeta de crédito se registran como cuotas mensuales que se cargarán automáticamente cada
            mes hasta completar el total de cuotas.
          </CardDescription>
        </CardHeader>
      </Card>
          ...
          <Card className="bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
            <CardContent className="pt-6">
              <div className="text-center">
                <div className="text-sm text-green-700 dark:text-green-300">Cuota mensual</div>
                <div className="text-2xl font-bold text-green-800 dark:text-green-200">${monthlyAmount}</div>
                <div className="text-sm text-green-600 dark:text-green-400">durante {installments} meses</div>
              </div>
            </CardContent>
          </Card>
        )}
